...
export type ExportType = 'settlement-transactions' | 'wallet-history' | 'payouts' | 'collections' | 'activities' | string;
...
export const useExportJob = () => {
    ...
    const [persistedJobs, setPersistedJobs] = useLocalStorage<Record<string, {
        jobId: string;
        exportType: ExportType;
        timestamp: number;
        statusEndpoint: string;
        ...
        const checkStatus = async (): Promise<void> => {
            ...
            try {
                ...
                if (response?.data?.progress) {
                    ...
                    const calculatedProgress = Math.min(90, (retries / maxRetries) * 100);
                ...
                if (response?.data?.status === 'failed') {
                    setState(prev => ({ ...prev, status: 'failed', error: response?.data?.message || 'Export failed' }));
                    notifyError(response?.data?.message || 'Export failed. Please try again.');
    ...
    const startExport = useCallback(async (options: ExportOptions) => {
        ...
        try {
            ...
            checkExportStatus(jobId.toString(), statusEndpoint, { onSuccess, pollingInterval, maxRetries });
    ...
    const resumeExport = useCallback((jobId: string, statusEndpoint: string) => {
