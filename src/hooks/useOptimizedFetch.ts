...
export function usePaginatedStoreQuery<T = any>(
  ...
  const loading =
    store[
      `get${actionName.charAt(0).toUpperCase() + actionName.slice(1)}Loading`
    ] || false;
  ...
  const queryResult = useStoreQuery(queryKey, action, actionParams, {
    ...options,
    cacheTime: 1000,
...
function cleanupStaleEntries() {
  ...
  requestCache.forEach((value, key) => {
    if (value.lastCall && now - value.lastCall > 30000) {
      // 30 seconds
