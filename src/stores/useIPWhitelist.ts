...
type IPWhitelistStore = {
  ...
  fetchIPWhitelist: (
    searchParams?: object
  ) => Promise<{ entries: IPWhitelistEntry[] }>;
  addIPWhitelist: (
...
const useIPWhitelist = create<IPWhitelistStore>((set, get) => ({
  ...
  // Remove IP from whitelist (otp required when user has TOTP enabled)
  removeIPWhitelist: async (id, otp) => {
    set((state) => ({ ...state, isLoading: true, error: null }));
    try {
      await deleteIPWhitelist(id, otp != null ? { otp } : undefined);
