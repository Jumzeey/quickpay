...
const SubAccountForm: React.FC = () => {
  ...
  const postDisbursement = async () => {
    ...
    const modeValue = formik.values.mode ? 1 : 0;
    ...
    try {
      ...
      setIsLoading(false);
      router.push('/your-business/sub-accounts');
    } catch (error: any) {
  ...
  return (
    <Layout pageTitle="Sub Account" icon="sub-accounts">
      <div className="flex justify-between items-center p-4 sm:p-6 lg:p-12">
        <Image src="/images/arrow-back.svg" className="cursor-pointer" width={36} height={36} onClick={() => router.back()} alt="back icon" />
      </div>
      <div className="flex justify-center mt-4 sm:mt-6 lg:mt-2">
        <Card extraPadding>
          <form onSubmit={formik.handleSubmit} className="space-y-4">
            <FloatingLabelInput label="Site Name" id="siteName" type="text" formik={formik} {...formik.getFieldProps('siteName')} />
            <FloatingLabelInput label="Website URL" id="websiteUrl" type="text" formik={formik} {...formik.getFieldProps('websiteUrl')} />
            <FloatingLabelInput label="Merchant Name" id="merchant_name" type="text" formik={formik} {...formik.getFieldProps('merchant_name')} />
            <FloatingLabelInput label="Percentage split" id="percentage" type="number" formik={formik} {...formik.getFieldProps('percentage')} />
            <FloatingLabelInput label="Contact Email" id="contactEmail" type="text" formik={formik} {...formik.getFieldProps('contactEmail')} />

            <div>
              <label className="font-semibold">Risk Rating</label>
              <select className="h-[60px] w-full rounded-lg border p-2" {...formik.getFieldProps('riskRating')}>
            ...
            <div>
              ...
              <Select
                ...
                value={formik.values.category ? { value: formik.values.category, label: formik.values.category } : null}
            ...
            <div>
              ...
              <FancyFileUpload documents={documents} setDocuments={setDocuments} />
            </div>

            <FloatingLabelInput label="Description" id="description" type="text" formik={formik} {...formik.getFieldProps('description')} />

            <Button className="text-white mt-4 text-xs sm:text-sm p-2 sm:p-3 rounded" text={isLoading ? <Loader /> : 'Create Sub Account'} disabled={!formik.isValid || isLoading} primary />
          </form>
        </Card>
      </div>
    </Layout>
