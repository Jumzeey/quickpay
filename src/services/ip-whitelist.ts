...
export async function getIPWhitelist(searchParams?: {}): Promise<IPWhitelistResponse> {
  try {
    const response = await api.get<IPWhitelistResponse>(
      apiEndpoints.ipWhitelist.GET_IP_WHITELIST
    );
    // @ts-ignore
    return response;
  } catch (error) {
...
// Delete IP whitelist entry (body with otp required when user has TOTP enabled)
